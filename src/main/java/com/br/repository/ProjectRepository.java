...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    @Query("SELECT p FROM Project p "
            + "LEFT JOIN p.members mb "
            + "WHERE ((:projectIds) IS NULL OR p.id NOT IN :projectIds) "
            + "AND ((:idUser) IS NULL OR mb.id IN :idUser) "
            + "AND ((:organizationId) IS NULL OR p.organization.id IN :organizationId) ")
